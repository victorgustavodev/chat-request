...
export default function DashboardPage() {
  ...
  const [filteredRequerimentos, setFilteredRequerimentos] = useState<ApiRequerimento[]>([]);
  ...
  const filterOptions = ['Todos', 'Em Análise', 'Deferido', 'Indeferido'];
  ...
  useEffect(() => {
    async function carregarDados() {
      try {
        ...
        const token = localStorage.getItem("token"); // Usa o token padrão
        if (!token) {
        ...
        setFilteredRequerimentos(dadosDaApi.data); // Inicializa a lista filtrada

      } catch (err: any) {
        setError(err.message || 'Ocorreu um erro desconhecido.');
        setTimeout(() => {
            router.push('/cradt/signin'); // Redireciona para o login de admin em caso de erro
        }, 3000);
      } finally {
        setIsLoading(false);
      }
  ...
  // Efeito para aplicar o filtro quando o status ou a lista principal mudar
  useEffect(() => {
    if (activeFilter === 'Todos') {
      setFilteredRequerimentos(requerimentos);
    } else {
      const filtrados = requerimentos.filter(req => req.status === activeFilter);
      setFilteredRequerimentos(filtrados);
    }
  }, [activeFilter, requerimentos]);
  ...
  const handleRowClick = (id: number) => {
    router.push(`/cradt/requerimentos/${id}`);
  };
      ...
      <div className="min-h-screen flex flex-col md:flex-row">
        ...
        >
          ...
          <button
            ...
            className="hover:bg-emerald-800 rounded-md transition whitespace-nowrap flex items-center justify-center cursor-pointer gap-2 p-3"
            title="Fazer Logout"
          >
            
            {sidebarOpen && <span>Fazer Logout</span>}
        ...
        {menuOpen && (
          <div className="md:hidden bg-[#002415] text-white p-4 flex flex-col gap-4">
            <button
              onClick={handleLogoutClick}
              className="hover:bg-emerald-800 cursor-pointer p-2 rounded transition text-left"
            >
              Sair
            </button>
          </div>
                ...
                {filteredRequerimentos.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={5} className="text-center py-8 text-gray-500">
                      Nenhum requerimento encontrado para o filtro "{activeFilter}".
                    </td>
                  </tr>
                )}
              </tbody>
