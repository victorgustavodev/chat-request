...
const FileUploadStep = ({ onSubmit }: { onSubmit: (file: File | null) => void }) => {
    ...
    return (
        <div className="w-full max-w-lg p-4 bg-white rounded-lg shadow-md flex flex-col gap-4 animate-fade-in-up">
            ...
            <div>
                ...
                <input
                    type="file" id="attachment" name="attachment" onChange={handleFileChange}
                    className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                />
            </div>
            <button onClick={handleSubmit} className="w-full bg-green-700 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-800 transition-colors">
...
const RequirementTypeStep = ({ allTypes, onSelect }: { allTypes: string[], onSelect: (type: string) => void }) => {
...
<div className="w-full max-w-2xl p-4 sm:p-6 bg-white rounded-xl shadow-lg flex flex-col gap-4 animate-fade-in-up">
            <div className="flex flex-col gap-2">
                <h3 className="text-lg font-bold text-gray-800">Qual o tipo de requerimento?</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {filteredTypes.length > 0 ? (
                        filteredTypes.map(type => (
                            <CardOption key={type} title={type} onClick={() => onSelect(type)} />
                        ))
                    ) : (
...
export default function Home() {
    ...
    const availableRegistrations = ["MAT-2025-001", "MAT-2025-002", "MAT-2025-003"];
    ...
    const handleInitialOption = (option: string) => {
        ...
        setTimeout(() => {
            if (option === "Solicitar Requerimento") {
                ...
                const botResponse = {
                    id: Date.now() + 1,
                    component: <div className="bg-white p-3 rounded-lg text-gray-800 max-w-xs shadow">A funcionalidade de consulta ainda está em desenvolvimento.</div>,
                    alignment: 'start'
                };
    ...
    const askForRequirementType = () => {
        const botMessage = {
            ...
            component: <RequirementTypeStep allTypes={requirementTypes} onSelect={handleTypeSelect} />,
            alignment: 'start'
        };
    ...
    const finalizeRequirement = (file: File | null) => {
        ...
        const confirmationMessage = {
            id: Date.now(),
            component: <div className="bg-white p-4 rounded-lg text-gray-800 max-w-md shadow text-center">✅<br/><strong>Requerimento enviado com sucesso!</strong><br/>Consulte o status na opção "Consultar Requerimentos".</div>,
            alignment: 'center'
        };
    ...
    useEffect(() => {
        function handleClickOutside(event: MouseEvent) {
            if (menuRef.current && !menuRef.current.contains(event.target as Node)) setIsMenuVisible(false);
        }
    ...
    if (!isAuthChecked) {
        return <div className="flex h-screen w-full items-center justify-center bg-[#002415] text-white">Carregando...</div>;
    }
    return (
        <div className="h-screen flex flex-col bg-[#D9D9D9]">
            <Navbar function={toggleMenu} />
            <div ref={menuRef}> <MenuNav isVisible={isMenuVisible} onClose={() => setIsMenuVisible(false)} /> </div>
            <main className="flex-1 overflow-y-auto p-4">
                <div className="flex flex-col gap-5 w-full max-w-3xl mx-auto">
                    {messages.map(msg => {
                        const containerClasses = `flex w-full justify-${msg.alignment || 'center'}`;
                        return (
                            <div key={msg.id} className={containerClasses}>
                                {msg.type === 'options' ? (
                                    <div className={`gap-4 w-full lg:w-3/4 animate-fade-in-up ${msg.layout || 'grid grid-cols-1 sm:grid-cols-2'}`}>
                                        {msg.options.map((opt: string) => (
                                            <CardOption key={opt} title={opt} onClick={() => msg.handler(opt)} />
                                        ))}
                                    </div>
                                ) : ( msg.component )}
                            </div>
